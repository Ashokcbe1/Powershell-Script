...
Expand-Archive "\\system Host Name\shared folder\zabbix_agent-5.0.3-windows-amd64-openssl.zip" "c:\Program Files\Zabbix-Agent"  


# Replaces 127.0.0.1 with your Zabbix server IP in the config file
(Get-Content -Path "c:\Program Files\Zabbix-Agent\conf\zabbix_agentd.conf") | ForEach-Object {$_ -Replace '127.0.0.1', "Zabbix Server IP"} | Set-Content -Path "c:\Program Files\Zabbix-Agent\conf\zabbix_agentd.conf"
